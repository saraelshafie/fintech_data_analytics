...
from db import fetch_column_statistics
...
def clean(fintech_df, isRow=False):
    ...
    if(isRow):
        stats_emp_title = fetch_column_statistics(column_name="emp_title", table_name="fintech_data_MET_P02_52_0812_clean")
        stats_emp_length = fetch_column_statistics(column_name="emp_length", table_name="fintech_data_MET_P02_52_0812_clean")
        stats_int_rate = fetch_column_statistics(column_name="int_rate", table_name="fintech_data_MET_P02_52_0812_clean")

        univariate_imputation(fintech_df, 'emp_title', stats_emp_title['mode'])
        univariate_imputation(fintech_df, 'emp_length', stats_emp_length['mean'])
        univariate_imputation(fintech_df, 'int_rate', stats_int_rate['mean'])
    else:
...
def fill_lookup_df(fintech_df, lookup_df):
    lookup_df = update_lookup_df(lookup_df, 'annual_inc_joint', 'missing', 0)

    mean = np.exp(fintech_df['int_rate']).mean()
    ...
    lookup_df = add_lookup_values(lookup_df, 'letter_grade', fintech_df['letter_grade'], fintech_df['letter_grade_encoded'])
    lookup_df = add_lookup_values(lookup_df, 'addr_state', fintech_df['addr_state'], fintech_df['addr_state_encoded'])
    lookup_df = add_lookup_values(lookup_df, 'state', fintech_df['state'], fintech_df['state_encoded'])
    lookup_df = add_lookup_values(lookup_df, 'purpose', fintech_df['purpose'], fintech_df['purpose_encoded'])
    lookup_df = add_lookup_values(lookup_df, 'loan_status', fintech_df['loan_status'], fintech_df['loan_status_encoded'])
    lookup_df = add_lookup_values(lookup_df, 'verification_status', fintech_df['verification_status'], fintech_df['verification_status_encoded'])
